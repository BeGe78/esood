...
#Successful tests  with language :en and :fr  
  test "logon_ok" do
    ...
    assert_selector('div#stripe_button', visible: false); puts("LogonTest::logon_ok assert stripe button not visible")
    find('#user_remember_me',visible:  true).click
    assert_selector('div#stripe_button', visible: true);  puts("LogonTest::logon_ok assert stripe button visible")
